...
@Slf4j
public class PostRequests extends GetRequests{
    File file = new File("src/main/resources/jsonEvgeny/issueComment.json");
    @Override
    public Response withToken(String token, int responseCode, String resources) {
        ValidateResponseWaiter responseWaiter = () -> {
            RestAssured.baseURI = TestConfigurations.getTestGitUri();
            return given()
        ...
        log.info("Response returned : {}", resp.prettyPrint());
